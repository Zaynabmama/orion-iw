...
def parse_args():
    parser = argparse.ArgumentParser(description="Sync KSA invoices for an invoice-date range.")
...
def main_run():
    ...
    config_path = Path(__file__).parent / "tenants" / "ksa_production.json"
    ...
    print(f"Running KSA invoices from {args.start} through the day before {args.end}{scope}")
    result = main.sync_tenant(config, orion, print)
    print("KSA RUN RESULT:", result)
