...
def parse_args():
    parser = argparse.ArgumentParser(description="Sync freezone invoices for an invoice-date range.")
...
def main_run():
    ...
    config_path = Path(__file__).parent / "tenants" / "freezone.json"
    ...
    print(f"Running freezone invoices from {args.start} through the day before {args.end}{scope}")
    result = main.sync_tenant(config, orion, print)
    print("FREEZONE RUN RESULT:", result)
