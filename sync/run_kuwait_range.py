...
def parse_args():
    parser = argparse.ArgumentParser(description="Sync kuwait invoices for an invoice-date range.")
...
def main_run():
    ...
    config_path = Path(__file__).parent / "tenants" / "kuwait.json"
    ...
    print(f"Running kuwait invoices from {args.start} through the day before {args.end}{scope}")
    result = main.sync_tenant(config, orion, print)
    print("KUWAIT RUN RESULT:", result)
